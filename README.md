...
4. **NCEP 最新 cycle 尚未完整**：只有候選目錄回傳 403，或目錄已列出但 tracker 檔案回傳 403／404 時，程式才會往前嘗試最多 8 個候選 cycle；可讀 tracker 檔案的其他 HTTP、網路或內容錯誤會立即回報失敗。若持續失敗，查看 Actions 紀錄中的來源訊息，稍後重試，並保留既有有效資料。
